...
;(function($$){ 'use strict';
  ...
  $$.fn.core({
    ...
    zoom: function( params ){
	  ...
	  //log("zoom:" + zoom + " maxZoom:"+this._private.maxZoom);
...
//    }
  ...
  CanvasRenderer.prototype.recalculateEdgeLabelProjection = function( edge ){
    ...
    if (rs.edgeType == 'self') {
      ...
      edgeCenterX = $$.math.qbezierAt( rs.startX, rs.cp2x, rs.endX, 0.4 );
      edgeCenterY = $$.math.qbezierAt( rs.startY, rs.cp2y, rs.endY, 0.4 );
    } else if (rs.edgeType == 'haystack') {
...
// Draw edge
  CanvasRenderer.prototype.drawEdge = function(context, edge, drawOverlayInstead) {
    ...
    if( rs.edgeType === 'haystack' ){
	  ...
	  // burda
	  if (false && context.lineWidth>2) {
		  var pako = context.strokeStyle;
		  var pako2 = context.lineWidth;
		  ...
		  this.drawStyledEdge(edge, context, [details.startX, details.startY,
			...
			lineStyle,
			edgeWidth);		
		  context.strokeStyle = pako;
		  context.lineWidth = pako2;
	  }	  
...
;(function($$){ 'use strict';
  ...
  CanvasRenderer.prototype.drawText = function(context, element, textX, textY) {
    ...
    if ( text != undefined && !isNaN(textX) && !isNaN(textY) ) {
	  ...
	  var fSize = style["font-size"].pxValue;
	  // SONER MULTILINE
	  ...
	  for (var m=0;m<lines.length;m++) {
        ...
        if (lineWidth > 0) {
          ...
          context.strokeText(line, textX,textY+ fSize*(m-(lines.length-1)/2) );
        }		
        context.fillText(line, textX, textY+ fSize*(m-(lines.length-1)/2) );
		/*
		if (lines.length>1 && m==0 && jQuery.isNumeric(line)) {
			var sizeImgThroughputIcon = 16;
			var imgThroughputIcon=document.getElementById('imgThroughputIcon');
			context.drawImage(imgThroughputIcon, textX - 2*sizeImgThroughputIcon,textY+fSize*(m-(lines.length-1)/2)-sizeImgThroughputIcon/2,sizeImgThroughputIcon,sizeImgThroughputIcon);
		}
		*/
    ...
    r.registerBinding(window, 'touchmove', $$.util.throttle(function(e) {
      ...
      if( capture && r.touchData.cxt ){
        ...
        if( factor != 1 && twoFingersStartInside){
          ...
          zoom2 = zoom2 > cy._private.maxZoom ? cy._private.maxZoom : zoom2;
          zoom2 = zoom2 < cy._private.minZoom ? cy._private.minZoom : zoom2;
...
;(function($$){ 'use strict';
  ...
  ArborLayout.prototype.run = function(){
	...
	if (typeof(options.grabHandler)!="undefined" && options.grabHandler!=null) {
		grabHandler = options.grabHandler;
	} else {
		grabHandler = function(e){		
			//console.log('grabo:'+cy._private.layout.running);
  ...
  ArborLayout.prototype.start = function(){
    if( this.system != null ){
		this.running = true;
      this.system.start();
    }
